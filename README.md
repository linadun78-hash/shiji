...
仓库当前提供的是本地开发者预览版。用户可以选择扩展直接连接 OpenAI 兼容模型服务，或自行启动 `127.0.0.1:8765` 的 FastAPI 本地模式并填写模型配置；它不是 Chrome Web Store 的安装即用版本。完整安装、打包和隐私边界请先阅读 [LOCAL_PREVIEW.md](LOCAL_PREVIEW.md)。
...
- `identity` 和 `https://*.auth0.com/*`：保留给下方默认关闭的云端登录试验；本地模式不使用这两个权限连接账号服务。
